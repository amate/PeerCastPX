...
//			GetWindowPlacement(guiWnd, &winPlace);
		...
		{ //JP-MOD
			...
			iniFile.writeBoolValue("guiMinimizeStore", servMgr->guiMinimizeStore);
			iniFile.writeStrValue("PlayerPath", servMgr->PlayerPath);
...
void ServMgr::loadSettings(const char *fn)
{
	...
	if (iniFile.openReadOnly(fn))
	{
		while (iniFile.readNext())
		{
			...
			else if (iniFile.isName("guiMinimizeStore"))
				servMgr->guiMinimizeStore = iniFile.getBoolValue();
			else if (iniFile.isName("PlayerPath"))
				servMgr->PlayerPath.set(iniFile.getStrValue());
