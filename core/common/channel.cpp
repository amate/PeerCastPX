...
bool	Channel::isFull()
{
	if (overrideMaxRelaysPerChannel > 0){ //JP-MOD 個別にリレー数設定
...
void ChanMgr::playChannel(ChanInfo &info)
{
	...
	if ((info.contentType == ChanInfo::T_WMA) || (info.contentType == ChanInfo::T_WMV)) {
		...
		if (servMgr->getModulePath) {
			peercastApp->getDirectory();
			//sprintf(fname,"%s/%s.asx",servMgr->modulePath,idStr);		// お試し
			sprintf(fname,"%s%s.asx",servMgr->modulePath,idStr);	// modulePathで最後に'\'が入るっぽい	
		} else {
		...
		if (servMgr->getModulePath) {
			peercastApp->getDirectory();
			sprintf(fname,"%splay.ram",servMgr->modulePath);
		} else
			sprintf(fname,"%splay.ram",peercastApp->getPath());

	} else {
		type = PlayList::T_SCPLS;
		if (servMgr->getModulePath) {
			peercastApp->getDirectory();
			sprintf(fname,"%splay.pls",servMgr->modulePath);
		} else
			sprintf(fname,"%splay.pls",peercastApp->getPath());
	}
...
#ifdef VERSION_EX
	memcpy(version_ex_prefix, PCP_CLIENT_VERSION_EX_PREFIX,2);
...
int	ChanHitList::numListeners()
{
	...
	{
		if (ch->host.ip && !ch->dead && ch->numHops)
			cnt += (unsigned int)ch->numListeners > 3 ? 3 : ch->numListeners; //JP-MOD リスナー数偽装防止
		ch=ch->next;
	}
